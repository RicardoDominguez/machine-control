...
class TensorStandardScaler:
    ...
    """
    def __init__(self, x_dim, name=0):
        ...
        with tf.variable_scope("Scaler"):
            ...
            self.sigma = tf.get_variable(
                name="scaler_std"+str(name), shape=[1, x_dim], initializer=tf.constant_initializer(1.0),
                trainable=False
            )
...
class TensorStandardScaler1D(TensorStandardScaler):
    """Helper class for automatically normalizing inputs into the network.
    """
    def __init__(self, name=0):
        ...
        with tf.variable_scope("Scaler"):
            ...
            self.sigma = tf.get_variable(
                name="scaler_std"+str(name), shape=[1], initializer=tf.constant_initializer(1.0),
                trainable=False
            )

        self.cached_mu, self.cached_sigma = np.array([0]), np.array([1])
