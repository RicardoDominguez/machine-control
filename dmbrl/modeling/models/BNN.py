...
from dmbrl.modeling.utils import ModelScaler
...
class BNN:
    ...
    def __init__(self, params):
        ...
        Arguments:
            params (DotMap): A dotmap of model parameters.
                ...
                .load_model (bxool): (optional) If True, model will be loaded from the model directory,
        ...
        """
        # Model name, used for logging/use in variable scopes.
        self.name = get_required_argument(params, 'name', 'Must provide name.')

        # Directoy of model if loaded
        ...
        # Instance variables
        self.finalized = False # Model being built or not
        self.layers = [] # No layers
        self.max_logvar, self.min_logvar = None, None # Regularize (prevent exploding or going to 0)
        self.decays = [] # Regularization, prevent very largue weights
        self.optvars = [] # ?
        self.nonoptvars = [] # ?
        self.end_act = None # ?
        self.end_act_name = None # ?
        self.scaler = None
        # Training objects
        self.optimizer = None
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        # Adam Optimizer with variable learning rate (mostly 1e-3)
        self.optimizer = optimizer(**optimizer_args)

        # Add variance output. (double output dimensions of the last layer ?)
        ...
        with self.sess.as_default():
            with tf.variable_scope(self.name):
                self.scaler = ModelScaler(self.layers[0].get_input_dim()) # ?
        ...
        if self.model_loaded:
            with self.sess.as_default():
                params_dict = loadPickle(os.path.join(self.model_dir, "%s.mat" % self.name))
    ...
    # -------------------------------------------------------------------------
    # All this are to record the train and holdout (validation data) losses to
    # see if the model is being overfit or underfit, and thus investigate what
    # choices of epochs (and even batch_size) work best

    def reset_train_losses(self, epochs):
        self.all_train_loss = np.zeros((epochs, self.num_nets))
        self.all_holdout_loss = np.zeros((epochs, self.num_nets))
        self.indx_record_train_loss = 0

    def update_train_losses(self, train_loss, holdout_loss):
        self.all_train_loss[self.indx_record_train_loss, :] = train_loss
        self.all_holdout_loss[self.indx_record_train_loss, :] = holdout_loss
        self.indx_record_train_loss += 1

    def return_train_losses(self):
        return self.all_train_loss, self.all_holdout_loss
    ...
    def train(self, inputs, targets,
        ...
        Returns: None
        """
        print("Batch size: %d, epochs: %d" % (batch_size, epochs))
        self.reset_train_losses(epochs)
        ...
        with self.sess.as_default():
            self.scaler.fit(inputs, targets)
        ...
        for _ in epoch_range:
            ...
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    ...
                    holdout_loss = self.sess.run(
                        self.mse_loss,
                        feed_dict={
                            self.sy_train_in: holdout_inputs,
                            self.sy_train_targ: holdout_targets
                        })
                    self.update_train_losses(training_loss, holdout_loss)
                    epoch_range.set_postfix({
                        "Training loss(es)": training_loss,
                        "Holdout loss(es)": holdout_loss
                        })
    ...
    def predict(self, inputs, factored=False, *args, **kwargs):
        ...
        if len(inputs.shape) == 2:
            if factored:
                # mean, var = self.sess.run([self.sy_pred_mean2d_fac, self.sy_pred_var2d_fac], feed_dict={self.sy_pred_in2d: inputs})
                return self.sess.run(
                    [self.sy_pred_mean2d_fac, self.sy_pred_var2d_fac],
                    feed_dict={self.sy_pred_in2d: inputs}
                )
            else:
                # mean, var = self.sess.run([self.sy_pred_mean2d, self.sy_pred_var2d], feed_dict={self.sy_pred_in2d: inputs})
                return self.sess.run(
                    [self.sy_pred_mean2d, self.sy_pred_var2d],
                    feed_dict={self.sy_pred_in2d: inputs}
                )
        else:
            # mean, var = self.sess.run([self.sy_pred_mean3d_fac, self.sy_pred_var3d_fac], feed_dict={self.sy_pred_in3d: inputs})
            return self.sess.run(
                [self.sy_pred_mean3d_fac, self.sy_pred_var3d_fac],
                feed_dict={self.sy_pred_in3d: inputs}
            )
        # return mean, np.zeros(var.shape)


    def create_prediction_tensors(self, inputs, factored=False, *args, **kwargs):
        """See predict() above for documentation.
        """
        f_mu, f_var = self._compile_outputs(inputs)
        factored_mean, factored_variance = self.scaler.inverse_transformOutput(f_mu, f_var)
        if inputs.shape.ndims == 2 and not factored:
            ...
            # return mean, tf.zeros(tf.shape(variance))
        return factored_mean, factored_variance
        # return factored_mean, tf.zeros(tf.shape(factored_variance) # factored_variance
    ...
    def save(self, savedir=None):
        ...
        savePickle(os.path.join(model_dir, "%s.mat" % self.name), var_vals)
    ...
    def _compile_outputs(self, inputs, ret_log_var=False):
        ...
        cur_out = self.scaler.transformInput(inputs)
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True):
        ...
        targets = self.scaler.transformTarget(targets)
